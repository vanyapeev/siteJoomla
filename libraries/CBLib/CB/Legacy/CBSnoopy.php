...
/*************************************************
 * $Id: Snoopy.class.php 891 2010-02-22 00:54:24Z beat $
...
class CBSnoopy
{
	...
	/*======================================================================*\
		...
		Purpose:	Set the form submission content type to
					text/xml
	\*======================================================================*/
	public function set_submit_xml()			//BB: function added.
	{
	...
	/*======================================================================*\
		...
		Purpose:	Set the form submission content type to
					text/json
	\*======================================================================*/
	public function set_submit_json()			//BB: function added.
	{
		$this->_submit_type = "text/json";
	}
	...
	private function _httpsrequest($url,$URI, /** @noinspection PhpUnusedParameterInspection */ $http_method,$content_type="",$body="")
	{
		if($this->passcookies && $this->_redirectaddr)
			$this->setcookies();
		...
		if(!empty($body))
			$headers[] = "Content-length: ".strlen($body);
		if(!empty($this->user) || !empty($this->pass))
			$headers[] = "Authorization: Basic ".base64_encode($this->user.":".$this->pass);		//BB: fixed BASIC -> Basic
		...
		$results	= null;
		$return		= null;
		$exec_line	= $this->curl_path." -D \"$headerfile\"".$cmdline_params." \"".$safer_URI."\"";
		exec($exec_line,$results,$return);	// add -k for non-certified
		if($return)
		{
