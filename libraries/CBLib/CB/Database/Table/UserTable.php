...
class UserTable extends ComprofilerTable
{
	...
	public function verifyPassword( $passwd ) {
		if ( checkJversion( '3.2+' ) ) {
			...
			return \JUserHelper::verifyPassword( $passwd, \JUser::getInstance( (int) $this->id )->password );
		} else {
			jimport( 'joomla.user.authentication' );

			$response		=	\JAuthentication::getInstance()->authenticate( array( 'username' => $this->username, 'password' => $passwd ) );
