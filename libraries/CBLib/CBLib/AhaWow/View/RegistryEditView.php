	...
	*/
				...
				if ( ( ( $paramsType == 'params' ) && $paramsName ) || ( $paramsType == 'pluginparams' ) ) {
					...
					if ( $repeat ) {
						...
						if ( $return ) {
							...
							if ( $viewType != 'view' ) {
								$return		=	'<div class="cbRepeat" data-cbrepeat-fallback-for="' . htmlspecialchars( $this->control_id( null, $parent_cname ) ) . '" data-cbrepeat-fallback-name="' . htmlspecialchars( $parent_cname ) . '"' . ( ! $repeatOrdering ? ' data-cbrepeat-sortable="false"' : null ) . ( $repeatIgnore ? ' data-cbrepeat-ignore="' . htmlspecialchars( $repeatIgnore ) . '"' : null ) . ( $repeatMax ? ' data-cbrepeat-max="' . (int) $repeatMax . '"' : null ) . '>'
											.		$return
	...
	protected function getTooltipAttr( $node, $attributes = null, $html = null ) {
		...
		if ( $title || $description ) {
			$return			=	cbTooltip( null, $description, trim( $title ), null, null, $html, null, $attributes );
	...
	function _form_yesno( $name, $value, $node, $control_name ) {
		...
		if ( $this->_view ) {
			...
			if ( $this->_isrepeat ) {
				...
				if ( $repeatDefault ) {
					$inputAttr		.=	' data-cbrepeat-default="' . htmlspecialchars( $repeatDefault ) . '"';
				}
			}

			$return				=	'<div' . ( $attributes ? ' ' . trim( $attributes ) : null ) . '>'
								.		'<input type="radio" name="' . htmlspecialchars( $this->control_name( $control_name, $name ) ) . '" id="' . htmlspecialchars( $this->control_id( $control_name, $name ) ) . '__yes" value="1" class="hidden"' . ( $value == 1 ? '  checked="checked"' : null ) . ( $inputAttr ? ' ' . $inputAttr : null ) . '>'
								.		'<label for="' . htmlspecialchars( $this->control_id( $control_name, $name ) ) . '__yes" class="btn btn-success">'
								.			$yes
								.		'</label>'
								.		'<input type="radio" name="' . htmlspecialchars( $this->control_name( $control_name, $name ) ) . '" id="' . htmlspecialchars( $this->control_id( $control_name, $name ) ) . '__no" value="0" class="hidden"' . ( $value == 0 ? '  checked="checked"' : null ) . ( $inputAttr ? ' ' . $inputAttr : null ) . '>'
