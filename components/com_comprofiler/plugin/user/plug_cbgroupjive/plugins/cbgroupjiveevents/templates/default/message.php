...
class HTML_groupjiveEventMessage
{
	 ...
	 */
	static function showMessage( $row, $input, $group, $user, $plugin )
	{
		...
		$showSubject				=	( ( $plugin->params->get( 'groups_events_message_type', 2, GetterInterface::INT ) == 1 ) && $plugin->params->get( 'groups_events_message_subject', false, GetterInterface::BOOLEAN ) );

		$return						.=	'<div class="gjEventMessage">'
									.		'<form action="' . $_CB_framework->pluginClassUrl( $plugin->element, true, array( 'action' => 'events', 'func' => 'send', 'id' => $row->get( 'id', 0, GetterInterface::INT ) ) ) . '" method="post" enctype="multipart/form-data" name="gjEventMessageForm" id="gjEventMessageForm" class="cb_form gjEventMessageForm form-auto cbValidation">'
									.			( $pageTitle ? '<div class="gjEventMessageTitle page-header"><h3>' . $pageTitle . '</h3></div>' : null );
		...
		if ( ( ! CBGroupJive::isModerator( $user->get( 'id' ) ) ) && $plugin->params->get( 'groups_events_message_captcha', 0 ) ) {
