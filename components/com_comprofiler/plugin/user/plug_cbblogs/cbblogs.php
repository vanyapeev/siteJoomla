...
class cbblogsTab extends cbTabHandler
{
	...
	{
		$title		=	parent::getTabTitle( $tab, $user, $ui, $postdata );

		if ( ! $this->params->get( 'tab_count', 1, GetterInterface::INT ) ) {
			return $title;
		}
		...
		return $title . ' <span class="badge badge-default">' . (int) $total . '</span>';
	}
	...
	public function getDisplayTab( $tab, $user, $ui )
	{
		...
		if ( $viewer->get( 'id' ) == $user->get( 'id' ) ) {
			if ( Application::User( (int) $viewer->get( 'id' ) )->isGlobalModerator() ) {
				$canCreate	=	true;
			} elseif ( Application::User( (int) $viewer->get( 'id' ) )->canViewAccessLevel( (int) $plugin->params->get( 'blog_create_access', 2 ) ) ) {
				$canCreate	=	true;
			}
		}
		...
		if ( ( ! $total ) && ( ! $searching ) && ( ! $canCreate ) && ( ! Application::Config()->get( 'showEmptyTabs', true, GetterInterface::BOOLEAN ) ) ) {
			return null;
		}
		...
		$rows				=	cbblogsModel::getBlogs( ( $this->params->get( 'tab_paging', 1 ) ? array( $pageNav->limitstart, $pageNav->limit ) : null ), $where, $this->params->get( 'tab_orderby', 'created,desc' ), $viewer, $user, $plugin );
...
class cbblogsPlugin extends cbPluginHandler
{
	...
	public function deleteBlogs( $user, /** @noinspection PhpUnusedParameterInspection */ $status )
	{
		...
		if ( $plugin->params->get( 'blog_delete', 1 ) ) {
			...
			$blogs		=	cbblogsModel::getBlogs( null, null, null, $user, $user, $plugin );
