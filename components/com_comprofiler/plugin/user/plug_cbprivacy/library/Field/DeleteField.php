...
class DeleteField extends \cbFieldHandler
{
	...
	public function getField( &$field, &$user, $output, $reason, $list_compare_types )
	{
		...
		if ( ( ! Application::Cms()->getClientId() ) && ( $output == 'htmledit' ) && ( $reason == 'edit' ) && $user->get( 'id', 0, GetterInterface::INT ) && ( ! Application::User( $user->get( 'id', 0, GetterInterface::INT ) )->isGlobalModerator() ) ) {
			...
			$value		=	'<a href="javascript: void(0);" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'Are you sure you want to delete your account?' ) ) . '\' ).done( function() { window.location.href = \'' . addslashes( $_CB_framework->pluginClassUrl( $this->element, true, array( 'action' => 'delete', 'id' => $user->get( 'id', 0, GetterInterface::INT ) ) ) ) . '\'; })">'
