...
class HTML_cbactivityCommentNew
{
	 ...
	 * @return string
	 */
	static public function showCommentNew( $viewer, $stream, $plugin, $output = null )
	{
		...
		$return					=	'<div class="streamItem commentContainer commentContainerNew' . ( $stream->get( 'inline', false, GetterInterface::BOOLEAN ) ? ' streamItemInline' : ' streamPanel panel' . ' panel-default' ) . ' streamItem' . $rowId . ( $isReply ? ' hidden' : null ) . '">'
								.		'<div class="streamItemInner">'
								.			'<form action="' . $_CB_framework->pluginClassUrl( $plugin->element, true, array( 'action' => 'comments', 'func' => 'save', 'stream' => $stream->id() ), 'raw', 0, true ) . '" method="post" enctype="multipart/form-data" name="streamItemForm' . $rowId . '" class="cb_form streamItemForm form">'
								...
								.						'<div class="streamItemNew border-default">'
								.							'<textarea name="message" rows="1" class="streamInput streamInputAutosize streamInputMessage' . ( $collapsed ? ' streamInputMessageCollapse' : null ) . ' form-control no-border" placeholder="' . htmlspecialchars( ( $isReply ? CBTxt::T( 'Write a reply...' ) : CBTxt::T( 'Write a comment...' ) ) ) . '"' . ( $messageLimit ? ' data-cbactivity-input-limit="' . (int) $messageLimit . '" maxlength="' . (int) $messageLimit . '"' : null ) . '></textarea>';

		if ( $showLinks ) {
			if ( ( ! $linkLimit ) || ( $linkLimit > 1 ) ) {
				$return			.=							'<div class="streamItemInputGroup streamInputLinkContainer cbRepeat border-default bg-default clearfix hidden" data-cbrepeat-sortable="false"' . ( $linkLimit ? ' data-cbrepeat-max="' . (int) $linkLimit . '"' : null ) . '>'
				...
				$return			.=							'<div class="streamItemInputGroup streamInputLinkContainer border-default bg-default clearfix hidden">'
		...
		if ( $actionOptions ) {
			...
			$return				.=							'<div class="streamItemInputGroup streamInputActionContainer border-default bg-default clearfix hidden">'
		...
		if ( $locationOptions ) {
			$return				.=							'<div class="streamItemInputGroup streamInputLocationContainer border-default bg-default clearfix hidden">'
								.								'<span class="streamItemInputGroupLabel streamInputSelectToggleLabel form-control"></span>'
								.								'<div class="streamItemInputGroupInput border-default">'
		...
		if ( $showTags ) {
			$tagsStream			=	new Tags( 'comment.0', $viewer->get( 'user_id', 0, GetterInterface::INT ) );
			...
			$return				.=							'<div class="streamItemInputGroup streamInputTagContainer border-default bg-default clearfix hidden">'
		...
		$return					.=							implode( '', $_PLUGINS->trigger( 'activity_onDisplayStreamCommentNew', array( &$buttons, $viewer, $stream, $output ) ) )
								.						'</div>'
								.						'<div class="streamItemDisplay commentContainerFooter' . ( $collapsed ? ' hidden' : null ) . '">'
		...
		$return					.=									( $buttons['right'] ? implode( ' ', $buttons['right'] ) . ' ' : null )
								.									'<button type="submit" class="commentsButton commentsButtonNewSave streamItemNewSave btn btn-primary btn-xs">' . CBTxt::T( 'Post' ) . '</button>'
								.									( $collapsed ? ' <button type="button" class="commentsButton commentsButtonNewCancel streamItemNewCancel btn btn-default btn-xs">' . CBTxt::T( 'Cancel' ) . '</button>' : null )
								.								'</div>'
								.							'</div>'
								.						'</div>'
								.					'</div>'
