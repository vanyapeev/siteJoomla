...
class TagTable extends Table
{
	...
	public function check()
	{
		if ( ! $this->get( 'user_id', 0, GetterInterface::INT ) ) {
			...
			return false;
		} elseif ( $this->get( 'tag', null, GetterInterface::STRING ) == '' ) {
			$this->setError( CBTxt::T( 'Tag not specified!' ) );
	...
	public function delete( $id = null )
	{
		...
		$query			=	'SELECT *'
						...
						.	"\n WHERE " . $this->getDbo()->NameQuote( 'asset' ) . " = " . $this->getDbo()->Quote( 'tag.' . $this->get( 'id', 0, GetterInterface::INT ) );
		...
		// Deletes comments about this tag:
		$query			=	'SELECT *'
		...
		// Deletes tag specific notifications:
		$query			=	'SELECT *'
