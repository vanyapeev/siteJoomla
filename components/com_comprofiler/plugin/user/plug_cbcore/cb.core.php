...
class CBfield_predefined extends CBfield_text {
	...
	protected function getDataAttributes( $field, $user, $output, $reason, $attributeArray = array() ) {
		...
		if ( $field->name == 'username' ) {
			...
			$attributeArray[]		=	cbValidator::getRuleHtmlAttributes( 'pattern', '/^[a-zA-Z][a-zA-Z0-9\-]+$/', CBTxt::T( 'VALIDATION_ERROR_FIELD_ALIAS', 'Please enter a valid alphanumeric profile url. Must begin with a letter and contain letters, numbers, and dashes only.' ) );
			$attributeArray[]		=	cbValidator::getRuleHtmlAttributes( 'cbfield', array( 'user' => (int) $user->id, 'field' => htmlspecialchars( $field->name ), 'reason' => htmlspecialchars( $reason ) ) );
		}
			...
			case 'htmledit':
				if ( $field->name == 'username' ) {
					...
					if ( ! Application::Cms()->getClientId() ) {
						...
						if ( is_numeric( $alias ) || ( $alias != Application::Router()->stringToAlias( $alias ) ) ) {
							$alias				=	$user->get( 'id', 0, GetterInterface::INT ) . '-' . Application::Router()->stringToAlias( $alias );
						}

						$return					.=	'<div class="cbCurrentProfileURL small text-muted">' . CBTxt::Th( 'YOUR_CURRENT_PROFILE_URL_IS_URL', 'Your current Profile URL is: [url]', array( '[url]' => $_CB_framework->viewUrl( 'userprofile', true, array( 'user' => $user->get( 'id', 0, GetterInterface::INT ) ) ), '[alias]' => $alias ) ) . '</div>';
					}
				...
				if ( $function == 'testexists' ) {
					...
					if ( $fieldName == 'alias' ) {
						if ( $_CB_database->isDbCollationCaseInsensitive() ) {
							$query		.=	"\n WHERE c." . $_CB_database->NameQuote( 'alias' ) . " = " . $_CB_database->Quote( trim( $value ) );
						} else {
							$query		.=	"\n WHERE LOWER( c." . $_CB_database->NameQuote( 'alias' ) . " ) = " . $_CB_database->Quote( strtolower( trim( $value ) ) );
						}
					} else {
						if ( $_CB_database->isDbCollationCaseInsensitive() ) {
							$query		.=	"\n WHERE u." . $_CB_database->NameQuote( 'username' ) . " = " . $_CB_database->Quote( trim( $value ) );
						} else {
							$query		.=	"\n WHERE LOWER( u." . $_CB_database->NameQuote( 'username' ) . " ) = " . $_CB_database->Quote( strtolower( trim( $value ) ) );
						}
					}
				} else {
					if ( $_CB_database->isDbCollationCaseInsensitive() ) {
						$query			.=	"\n WHERE ( u." . $_CB_database->NameQuote( 'username' ) . " = " . $_CB_database->Quote( trim( $value ) )
										.	' OR c.' . $_CB_database->NameQuote( 'alias' ) . ' = ' . $_CB_database->Quote( trim( $value ) ) . ' )';
					} else {
						$query			.=	"\n WHERE ( LOWER( u." . $_CB_database->NameQuote( 'username' ) . " ) = " . $_CB_database->Quote( strtolower( trim( $value ) ) )
										.	' OR LOWER( c.' . $_CB_database->NameQuote( 'alias' ) . ' ) = ' . $_CB_database->Quote( strtolower( trim( $value ) ) ) . ' )';
					}
				}
	...
	public function validate( &$field, &$user, $columnName, &$value, &$postdata, $reason ) {
		...
		if ( $validated && in_array( $fieldName, array( 'username', 'alias' ) ) ) {
			if ( $fieldName == 'alias' ) {
				if ( is_numeric( $value ) || preg_match( '/(^[^a-zA-Z])|[^a-zA-Z0-9\-]/', $value ) ) {
		...
		if ( $output == 'htmledit' ) {
			if ( ( $field->name != 'password' ) || ( $reason != 'register' ) || ! ( isset( $ueConfig['emailpass'] ) && ( $ueConfig['emailpass'] == "1" ) ) ) {
				if ( $field->params->get( 'fieldVerifyInput', 1 ) ) {
					...
					$descOfVerifyField					=	$field->params->get( 'verifyPassDesc', 'Please verify your new password.' );

					if ( $descOfVerifyField ) {
						$verifyField->description		=	$descOfVerifyField;
					}
				...
				if ( $verifyCurr ) {
					...
					$verifyCurrField->name				=	$field->name . '__current';
					$verifyCurrField->fieldid			=	$field->fieldid . '__current';

					$titleOfVerifyCurrentField			=	$field->params->get( 'verifyCurrentTitle' );
					$descOfVerifyCurrentField			=	$field->params->get( 'verifyCurrentDesc', 'Please verify your current password.' );

					if ( $titleOfVerifyCurrentField ) {
						$verifyCurrField->title			=	CBTxt::Th( $titleOfVerifyCurrentField, null, array( '[title]' => CBTxt::T( $field->title ) ) );
					} else {
						$verifyCurrField->title			=	CBTxt::Th( 'VERIFY_CURRENT_SOMETHING', 'Current [title]', array( '[title]' => CBTxt::T( $field->title ) ) );
					}

					if ( $descOfVerifyCurrentField ) {
						$verifyCurrField->description	=	$descOfVerifyCurrentField;
					}

					$verifyCurrField->_requiredIf		=	$field->name;
				}
		...
		switch ( $output ) {
			...
			case 'htmledit':
				...
				if ( ! isset( $fieldOptions[$cacheId] ) ) {
					$_CB_database->setQuery( "SELECT fieldtitle AS `value`, if ( fieldlabel != '', fieldlabel, fieldtitle ) AS `text`, `fieldgroup` AS `group`, concat('cbf',fieldvalueid) AS id FROM #__comprofiler_field_values"		// id needed for the labels
...
//					$html			=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', 'multicheckbox', $value, '', $allValues );
					...
					$html			=	$this->_fieldSearchModeHtml( $field, $user, $html, ( ( strpos( $displayType, 'multi' ) === 0 ) && ( ! in_array( $field->type, array( 'radio', 'select' ) ) ) ? 'multiplechoice' : 'singlechoice' ), $list_compare_types );
				} else {
					$html			=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', $field->type, $value, ( in_array( $field->type, array( 'multicheckbox', 'multiselect' ) ) ? $this->getDataAttributes( $field, $user, $output, $reason ) : '' ), $allValues );
				}
		...
		switch ( $output ) {
			...
			case 'rss':
				if ( ( $value != null ) && ( $value != '' ) && ( $value != '0000-00-00 00:00:00' ) && ( $value != '0000-00-00' ) ) {
					$offset					=	( $field->get( 'type', null, GetterInterface::STRING ) == 'datetime' ? $field->params->get( 'date_offset', true, GetterInterface::BOOLEAN ) : false );
			...
			case 'htmledit':
				...
				if ( $reason == 'search' ) {
					if ( $searchBy == 2 ) {
						$dateFormat			=	$field->params->get( 'custom_date_search_format', 'Y-m-d', GetterInterface::STRING );

						if ( $field->get( 'type', null, GetterInterface::STRING ) == 'datetime' ) {
							$timeFormat		=	$field->params->get( 'custom_time_search_format', 'H:i:s', GetterInterface::STRING );
						}
					}
				} else {
					if ( $field->params->get( 'field_edit_format', false, GetterInterface::BOOLEAN ) ) {
						$dateFormat			=	$field->params->get( 'custom_date_edit_format', 'Y-m-d', GetterInterface::STRING );

						if ( $field->get( 'type', null, GetterInterface::STRING ) == 'datetime' ) {
							$timeFormat		=	$field->params->get( 'custom_time_edit_format', 'H:i:s', GetterInterface::STRING );
						}
					}
				}
				...
				if ( $reason == 'search' ) {
					...
					if ( $searchBy == 1 ) {
						...
						$minHtml			=	$this->formatFieldValueLayout( $calendars->cbAddCalendar( $minNam, CBTxt::Th( 'UE_SEARCH_FROM', 'Between' ) . ' ' . $this->getFieldTitle( $field, $user, 'text', $reason ), false, $minVal, false, ( $field->get( 'type' ) == 'datetime' ? true : false ), $yMin, $yMax, $tooltip, $offset ), $reason, $field, $user );
						$maxHtml			=	$this->formatFieldValueLayout( $calendars->cbAddCalendar( $maxNam, CBTxt::Th( 'UE_SEARCH_TO', 'and' ) . ' ' . $this->getFieldTitle( $field, $user, 'text', $reason ), false, $maxVal, false, ( $field->get( 'type' ) == 'datetime' ? true : false ), $yMin, $yMax, $tooltip, $offset ), $reason, $field, $user );
					}
...
class CBfield_image extends cbFieldHandler {
	...
	function _avatarHtml( &$field, &$user, $reason, $thumbnail = true, $showAvatar = 2 ) {
		...
		if ( $field->name == 'canvas' ) {
			...
			if ( $profileLink && ( ! in_array( $reason, array( 'profile', 'edit' ) ) ) ) {
	...
	function _htmlEditForm( &$field, &$user, $reason, $displayFieldIcons = true ) {
		...
		if ( count( $choices ) > 1 ) {
			...
			if ( ! $functOut ) {
				$js								=	"function cbslideImageFile( choice, name ) {"
												...
												.			"$( '#cbimagefile_upload_' + name + ',#cbimagefile_gallery_' + name + ',#cbimagefile_position_' + name ).slideUp( 'slow' );"
												.			"$( '#cbimagefile_upload_' + name + ' input,#cbimagefile_gallery_' + name + ' input,#cbimagefile_position_' + name + ' input' ).prop( 'disabled', true );"
												.		"} else if ( choice == 'upload' ) {"
												.			"$( '#cbimagefile_gallery_' + name + ',#cbimagefile_position_' + name ).slideUp( 'slow' );"
												.			"$( '#cbimagefile_upload_' + name ).slideDown( 'slow' );"
												.			"$( '#cbimagefile_gallery_' + name + ' input,#cbimagefile_position_' + name + ' input' ).prop( 'disabled', true );"
												.			"$( '#cbimagefile_upload_' + name + ' input' ).prop( 'disabled', false );"
												.		"} else if ( choice == 'gallery' ) {"
												.			"$( '#cbimagefile_upload_' + name + ',#cbimagefile_position_' + name ).slideUp( 'slow' );"
												.			"$( '#cbimagefile_gallery_' + name ).slideDown( 'slow' );"
												.			"$( '#cbimagefile_upload_' + name + ' input,#cbimagefile_position_' + name + ' input' ).prop( 'disabled', true );"
												.			"$( '#cbimagefile_gallery_' + name + ' input' ).prop( 'disabled', false );"
												.		"} else if ( choice == 'position' ) {"
												.			"$( '#cbimagefile_upload_' + name + ',#cbimagefile_gallery_' + name ).slideUp( 'slow' );"
												.			"$( '#cbimagefile_position_' + name ).slideDown( 'slow' );"
												.			"$( '#cbimagefile_position_' + name ).find( '.cbCanvasRepositionSelect' ).draggable({"
												.				"containment: 'parent',"
												.				"scroll: false,"
												.				"axes: 'y',"
												.				"create: function() {"
												...
												.						"width: '100%'"
												.					"});"
												.					"var top = $( '#cbimagefile_position_' + name + ' input' ).val();"
												.					"if ( top != '' ) {"
												.						"if ( top < 0 ) {"
												.							"top = 0;"
												.						"} else if ( top > 100 ) {"
												...
												.				"},"
												.				"stop: function( e, ui ) {"
												.					"$( '#cbimagefile_position_' + name + ' input' ).val( ( 100 / ( ( $( this ).parent().height() - $( this ).height() ) / ui.position.top ) ).toFixed( 0 ) );"
												.				"}"
												.			"});"
												.			"$( '#cbimagefile_upload_' + name + ' input,#cbimagefile_gallery_' + name + ' input' ).prop( 'disabled', true );"
												.			"$( '#cbimagefile_position_' + name + ' input' ).prop( 'disabled', false );"
												.		"}"
												.	"}";
			...
			$js									=	"$( '#cbimagefile_upload_" . addslashes( $fieldName ) . ",#cbimagefile_gallery_" . addslashes( $fieldName ) . ",#cbimagefile_position_" . addslashes( $fieldName ) . "' ).hide();"
												.	"$( '#cbimagefile_upload_" . addslashes( $fieldName ) . " input,#cbimagefile_gallery_" . addslashes( $fieldName ) . " input,#cbimagefile_position_" . addslashes( $fieldName ) . " input' ).prop( 'disabled', true );"
		...
		if ( $field->params->get( 'image_allow_uploads', 1 ) ) {
			...
			if ( $field->params->get( 'image_client_resize', 1, GetterInterface::INT ) && ( $uploadWidthLimit || $uploadHeightLimit ) ) {
				$validationAttributes[]			=	cbValidator::getRuleHtmlAttributes( 'resize', array( $uploadWidthLimit, $uploadHeightLimit, $this->_getImageFieldParam( $field, 'avatarMaintainRatio', 1 ) ) );
			}
		...
		if ( $canReposition ) {
			...
			$return								.=	'<div id="cbimagefile_position_' . htmlspecialchars( $fieldName ) . '" class="form-group cb_form_line clearfix">'
												...
												.		'</div>'
												.		'<input type="hidden" name="' . htmlspecialchars( $fieldName ) . '__position" value="' . $position . '" />'
												.	'</div>';
		}
				...
				case 'rss':
					...
					if ( $profileLink && ( $reason != 'profile') ) {
						...
						if ( $field->params->get( 'fieldHoverCanvas', false, GetterInterface::BOOLEAN ) ) {
							...
							$tooltip			=	'<div class="cbCanvasBox cbCanvasBoxSm img-thumbnail">'
							...
							if ( $canvasContent ) {
								$tooltip		.=				$cbUser->replaceUserVars( $canvasContent );
							} else {
								$tooltip		.=				$cbUser->getField( 'onlinestatus', null, 'html', 'none', 'profile', 0, true, array( 'params' => array( 'displayMode' => 1 ) ) )
												.				' ' . $cbUser->getField( 'formatname', null, 'html', 'none', 'list', 0, true, array( 'params' => array( 'fieldHoverCanvas' => false ) ) );
							}

							$tooltip			.=			'</div>'
												.			( $canvasPlugins ? '<div class="cbCanvasBoxRow text-nowrap text-overflow">' . implode( '', $canvasPlugins ) . '</div>' : null )
												.		'</div>'
												.	'</div>';
...
class CBfield_userparams extends cbFieldHandler {
	...
	public function getFieldRow( &$field, &$user, $output, $formatting, $reason, $list_compare_types ) {
		...
		if ( is_array( $userParams ) && ( count( $userParams ) > 0 ) && ( ( $ui == 2 ) || ( ( isset( $ueConfig['frontend_userparams'] ) ) ? ( $ueConfig['frontend_userparams'] == 1 ) : in_array( $_CB_framework->getCfg( 'frontend_userparams' ), array( '1', null ) ) ) ) ) {
			...
			// Two factor authentication params:
			if ( checkJversion( '3.2+' ) && $user->id ) {
				...
				if ( count( $twoFactorMethods ) > 1 ) {
					...
					$paramField						=	new FieldTable( $field->getDbo() );
					$paramField->title				=	( isset( $userParam[2] ) && class_exists("JText") ? JText::_( 'COM_USERS_PROFILE_TWOFACTOR_LABEL' ) : null );
					$paramField->_html				=	$twoFactor;
					$paramField->description		=	( isset( $userParam[2] ) && class_exists("JText") ? JText::_( 'COM_USERS_PROFILE_TWOFACTOR_DESC' ) : null );
		...
		if ( $ui == 2 ) {
			...
			if ( $canBlockUser ) {
				...
				$lists['block']					=	moscomprofilerHTML::yesnoSelectList( 'block', 'class="form-control"', (string) $user->block, CBTxt::T( 'No' ), CBTxt::T( 'Yes' ) );
				...
				$lists['confirmed']				=	moscomprofilerHTML::yesnoSelectList( 'confirmed', 'class="form-control"', (string) $user->confirmed );
				...
				if ( checkJversion( '3.2+' ) ) {
					$lists['requireReset']		=	moscomprofilerHTML::yesnoSelectList( 'requireReset', 'class="form-control"', (string) $user->requireReset );
				}
				...
				if ( count( $twoFactorMethods ) > 1 ) {
					...
					if ( $twoFactorMethod != 'none' ) {
						if ( cbGetParam( $postdata, 'jform[twofactor][' . $twoFactorMethod . '][securitycode]' ) !== null ) {
							...
							$model->setOtpConfig( $user->id, $otpConfig );

							// Generate one time emergency passwords if required (depleted or not set)
							if ( empty( $otpConfig->otep ) ) {
								$model->generateOteps( $user->id );
							}
		...
		if ( count( $choices ) > 1 ) {
			...
			if ( ! $functOut ) {
				$js							=	"function cbslideFile( choice, name ) {"
											...
											.			"$( '#cbfile_upload_' + name + ' input' ).prop( 'disabled', true );"
											.		"} else if ( choice == 'upload' ) {"
											.			"$( '#cbfile_upload_' + name ).slideDown( 'slow' );"
											.			"$( '#cbfile_upload_' + name + ' input' ).prop( 'disabled', false );"
											.		"}"
											.	"}";
			...
			$js								=	"$( '#cbfile_upload_" . addslashes( $fieldName ) . "' ).hide();"
											.	"$( '#cbfile_upload_" . addslashes( $fieldName ) . " input' ).prop( 'disabled', true );"
											.	"$( '#" . addslashes( $fieldName ) . "__choice' ).on( 'click change', function() {"
...
class CBfield_video extends CBfield_text {
	...
	function _htmlEditForm( &$field, &$user, $output, $reason, $displayFieldIcons = true ) {
		...
		if ( count( $choices ) > 1 ) {
			...
			if ( ! $functOut ) {
				$js							=	"function cbslideVideoFile( choice, name ) {"
											...
											.			"$( '#cbvideofile_upload_' + name + ' input,#cbvideofile_link_' + name + ' input' ).prop( 'disabled', true );"
											.		"} else if ( choice == 'upload' ) {"
											.			"$( '#cbvideofile_link_' + name ).slideUp( 'slow' );"
											.			"$( '#cbvideofile_upload_' + name ).slideDown( 'slow' );"
											.			"$( '#cbvideofile_link_' + name + ' input' ).prop( 'disabled', true );"
											.			"$( '#cbvideofile_upload_' + name + ' input' ).prop( 'disabled', false );"
											.		"} else if ( choice == 'link' ) {"
											.			"$( '#cbvideofile_upload_' + name ).slideUp( 'slow' );"
											.			"$( '#cbvideofile_link_' + name ).slideDown( 'slow' );"
											.			"$( '#cbvideofile_upload_' + name + ' input' ).prop( 'disabled', true );"
											.			"$( '#cbvideofile_link_' + name + ' input' ).prop( 'disabled', false );"
											.		"}"
											.	"}";
			...
			$js								=	"$( '#cbvideofile_upload_" . addslashes( $fieldName ) . ",#cbvideofile_link_" . addslashes( $fieldName ) . "' ).hide();"
											.	"$( '#cbvideofile_upload_" . addslashes( $fieldName ) . " input,#cbvideofile_link_" . addslashes( $fieldName ) . " input' ).prop( 'disabled', true );"
											.	"$( '#" . addslashes( $fieldName ) . "__choice' ).on( 'click change', function() {"
		...
		if ( $field->params->get( 'video_allow_links', 1 ) ) {
			$return							.=	'<div id="cbvideofile_link_' . htmlspecialchars( $fieldName ) . '" class="form-group cb_form_line clearfix">'
											.		'<div>';
...
class CBfield_audio extends CBfield_text {
	...
	function _htmlEditForm( &$field, &$user, $output, $reason, $displayFieldIcons = true ) {
		...
		if ( count( $choices ) > 1 ) {
			...
			if ( ! $functOut ) {
				$js							=	"function cbslideAudioFile( choice, name ) {"
											...
											.			"$( '#cbaudiofile_upload_' + name + ' input,#cbaudiofile_link_' + name + ' input' ).prop( 'disabled', true );"
											.		"} else if ( choice == 'upload' ) {"
											.			"$( '#cbaudiofile_link_' + name ).slideUp( 'slow' );"
											.			"$( '#cbaudiofile_upload_' + name ).slideDown( 'slow' );"
											.			"$( '#cbaudiofile_link_' + name + ' input' ).prop( 'disabled', true );"
											.			"$( '#cbaudiofile_upload_' + name + ' input' ).prop( 'disabled', false );"
											.		"} else if ( choice == 'link' ) {"
											.			"$( '#cbaudiofile_upload_' + name ).slideUp( 'slow' );"
											.			"$( '#cbaudiofile_link_' + name ).slideDown( 'slow' );"
											.			"$( '#cbaudiofile_upload_' + name + ' input' ).prop( 'disabled', true );"
											.			"$( '#cbaudiofile_link_' + name + ' input' ).prop( 'disabled', false );"
											.		"}"
											.	"}";
			...
			$js								=	"$( '#cbaudiofile_upload_" . addslashes( $fieldName ) . ",#cbaudiofile_link_" . addslashes( $fieldName ) . "' ).hide();"
											.	"$( '#cbaudiofile_upload_" . addslashes( $fieldName ) . " input,#cbaudiofile_link_" . addslashes( $fieldName ) . " input' ).prop( 'disabled', true );"
											.	"$( '#" . addslashes( $fieldName ) . "__choice' ).on( 'click change', function() {"
		...
		if ( $field->params->get( 'audio_allow_links', 1 ) ) {
			$return							.=	'<div id="cbaudiofile_link_' . htmlspecialchars( $fieldName ) . '" class="form-group cb_form_line clearfix">'
											.		'<div>';
	...
	public function getField( &$field, &$user, $output, $reason, $list_compare_types )
	{
		...
		if ( ( $output == 'htmledit' ) && ( $reason != 'search' ) ) {
			...
			if ( $termsDisplay == 'iframe' ) {
				...
				$label					=	CBTxt::Th( 'TERMS_FIELD_I_AGREE_ON_THE_ABOVE_CONDITIONS FIELD_' . $field->get( 'fieldid', 0, GetterInterface::INT ) . '_TERMS_FIELD_I_AGREE_ON_THE_ABOVE_CONDITIONS', 'I Agree to the above [type].', array( '[type]' => $termsType ) );
			} else {
				$attributes				=	' class="cbTermsLink"';
				...
				$label					=	CBTxt::Th( 'TERMS_FIELD_ACCEPT_URL_CONDITIONS FIELD_' . $field->get( 'fieldid', 0, GetterInterface::INT ) . '_TERMS_FIELD_ACCEPT_URL_CONDITIONS', 'Accept <!--suppress HtmlUnknownTarget --><a href="[url]"[attributes]>[type]</a>', array( '[url]' => $url, '[attributes]' => $attributes, '[type]' => $termsType ) );
			}
