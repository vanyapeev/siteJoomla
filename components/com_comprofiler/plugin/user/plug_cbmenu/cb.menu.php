...
class getMenuTab extends cbTabHandler
{
	...
	public function getMenuAndStatus( $tab, $user, $ui )
	{
		...
		if ( ( $ueConfig['allow_email_display'] != 4 ) && ( $_CB_framework->myId() != $user->id ) && ( ( $_CB_framework->myId() > 0 ) || Application::Config()->get( 'allow_email_public', false, GetterInterface::BOOLEAN ) ) ) {
			switch ( $ueConfig['allow_email_display'] ) {
				...
				default:
					...
					$url				=	$_CB_framework->viewUrl( 'emailuser', true, array( 'uid' => $userId ), 'html', 0, '&uid=' . $userId );
					$desc				=	CBTxt::T( 'UE_MENU_SENDUSEREMAIL_DESC', 'Send an Email to this user' );
					break;
			}
			...
			if ( $_CB_framework->myId() != $user->id ) {
				...
				if ( ! $isConnection ) {
					...
					if ( $ueConfig['conNotifyType'] != 0 ) {
						...
						$connLink					=	cbTooltip( $ui, $tooltip, $tooltipTitle, 400, null, '<span class="fa fa-heart"></span> ' . CBTxt::Th( $connMsg ), 'javascript: void( 0 );', 'class="cbMenuRequestConnection" data-hascbtooltip="true" data-cbtooltip-modal="true" data-cbtooltip-open-target=".cbMenuRequestConnection"' );
					} else {
						$connLink					=	$connUrl;
