...
class CBProfileView_html_default extends cbProfileView
{
	...
	protected function _renderProfile( )
	{
		...
		if ( $canvasMenu || $canvasHeader || $canvasMain ) {
			...
			if ( $canvasHeader ) {
				$return					.=	'<div class="cbPosCanvas cbCanvasLayout border-default">';

				if ( $canvasBackground || $canvasPhoto ) {
					$return				.=		'<div class="cbPosCanvasTop cbCanvasLayoutTop">';

					if ( $canvasBackground ) {
						$return			.=			'<div class="cbPosCanvasBackground cbCanvasLayoutBackground bg-muted">'
										.				$canvasBackground
										.			'</div>';
					}

					if ( $canvasPhoto ) {
						$return			.=			'<div class="cbPosCanvasPhoto cbCanvasLayoutPhoto">'
										.				$canvasPhoto
										.			'</div>';
					}

					if ( $canvasInfo ) {
						$return			.=			'<div class="cbPosCanvasInfo cbCanvasLayoutButtons text-right">'
										.				$canvasInfo
										.			'</div>';
					}
				...
				if ( $canvasStats || $canvasTitle ) {
					$return				.=		'<div class="cbPosCanvasBottom cbCanvasLayoutBottom border-default">';

					if ( $canvasTitle ) {
						$return			.=			'<div class="cbPosCanvasTitle cbCanvasLayoutTitle text-primary">'
										.				$canvasTitle
										.			'</div>';
					}

					if ( $canvasStats ) {
						$return			.=			'<div class="cbPosCanvasStats cbCanvasLayoutCounters text-muted small">'
										.				$canvasStats
										.			'</div>';
					}
			...
			if ( $canvasMain ) {
				...
				$return					.=	'<div class="cbPosCanvasMain cbCanvasLayoutMain clearfix">'
...
class CBListView_html_default extends cbListView
{
	...
	protected function _renderHead( )
	{
		...
		if ( $this->totalIsAllUsers ) {
			...
			$return					.=					CBTxt::Th( 'SITENAME_HAS_TOTAL_REGISTERED_MEMBERS USERLIST_' . (int) $this->listId . '_TOTAL_REGISTERED_MEMBERS', '[SITENAME] has %%TOTAL%% registered member|[SITENAME] has %%TOTAL%% registered members', array( '[SITENAME]' => $_CB_framework->getCfg( 'sitename' ), '[title]' => $this->listTitleHtml, '%%TOTAL%%' => $this->total ) );
		} else {
			...
			$return					.=					CBTxt::Th( 'USERS_COUNT_MEMBERS USERLIST_' . (int) $this->listId . '_COUNT_MEMBERS', '%%USERS_COUNT%% member|%%USERS_COUNT%% members', array( '[SITENAME]' => $_CB_framework->getCfg( 'sitename' ), '[title]' => $this->listTitleHtml, '%%USERS_COUNT%%' => $this->total ) );
		}
			...
			if ( $this->searchTabContent ) {
				if ( ( ! $this->searchResultDisplaying ) || $this->searchCollapsed ) {
					$return			.=						'<div class="cbUserListSearchButtons cbUserListsSearchTrigger">'
									...
									.							'<button type="button" class="btn btn-default btn-block cbUserListsSearchButton">' . CBTxt::Th( 'UE_SEARCH_USERS USERLIST_' . (int) $this->listId . '_SEARCH_USERS', 'Search Users', array( '[title]' => $this->listTitleHtml ) ) . ' <span class="fa fa-caret-down"></span></button>'
									.						'</div>';
				}
		...
		if ( $this->searchTabContent ) {
			$return					.=	'<div class="cbUserListSearch">'
									...
									.					'<input type="submit" class="btn btn-primary cbUserlistSubmit" value="' . CBTxt::Th( 'UE_FIND_USERS USERLIST_' . (int) $this->listId . '_FIND_USERS', 'Find Users', array( '[title]' => $this->listTitleHtml ) ) . '"' . cbValidator::getSubmitBtnHtmlAttributes() . ' />';
		...
		if ( is_array( $this->users ) && ( count( $this->users ) > 0 ) ) foreach ( $this->users as $userIndex => $user ) {
			...
			if ( $formatting == 'divs' ) {
				...
				$return			.=			'<div class="sectiontableentry1' . ( $layout != 'grid' ? ' cbColumn12 col-sm-12' : null ) . '">'
								...
								.				CBTxt::Th( 'UE_NO_USERS_IN_LIST USERLIST_' . (int) $this->listId . '_NO_USERS_IN_LIST', 'No users in this list', array( '[title]' => $this->listTitleHtml ) )
								.			'</div>';
				...
				$return			.=			'<tr class="sectiontableentry1">'
																						// CBTxt::Th( 'UE_NO_USERS_IN_LIST', 'No users in this list' )
								.				'<td colspan="' . $columnCount . '">' . CBTxt::Th( 'UE_NO_USERS_IN_LIST USERLIST_' . (int) $this->listId . '_NO_USERS_IN_LIST', 'No users in this list', array( '[title]' => $this->listTitleHtml ) ) . '</td>'
								.			'</tr>';
			}
		}
