...
class ActivityField extends \cbFieldHandler
{
	...
	public function getField( &$field, &$user, $output, $reason, $list_compare_types )
	{
		...
		$asset			=	str_replace( '[field_id]', $field->get( 'fieldid', 0, GetterInterface::INT ), $field->params->get( 'activity_asset', null, GetterInterface::STRING ) );
		...
		$activity->parse( $field->params, 'activity_' );

		$activity->set( 'field', $field->get( 'fieldid', 0, GetterInterface::INT ) );

		if ( ( ! Application::Config()->get( 'showEmptyFields', 1, GetterInterface::INT ) ) && ( ! $activity->rows( 'count' ) ) && ( ! CBActivity::canCreate( 'activity', $activity ) ) ) {
			return cbReplaceVars( Application::Config()->get( 'emptyFieldsText', null, GetterInterface::STRING ), $user );
		}

		if ( in_array( $field->params->get( 'activity_layout', 'stream', GetterInterface::STRING ), array( 'button', 'toggle' ) ) ) {
