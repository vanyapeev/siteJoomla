...
function performCheckUsername( $username, $function ) {
	...
	if ( $function == 'testexists' ) {
		if ( $_CB_database->isDbCollationCaseInsensitive() ) {
			...
			$query	.=	"\n WHERE LOWER( u." . $_CB_database->NameQuote( 'username' ) . " ) = " . $_CB_database->Quote( strtolower( trim( $username ) ) );
		}
	} else {
		if ( $_CB_database->isDbCollationCaseInsensitive() ) {
			$query	.=	"\n WHERE ( u." . $_CB_database->NameQuote( 'username' ) . " = " . $_CB_database->Quote( trim( $username ) )
			...
			$query	.=	"\n WHERE ( LOWER( u." . $_CB_database->NameQuote( 'username' ) . " ) = " . $_CB_database->Quote( strtolower( trim( $username ) ) )
					.	' OR LOWER( c.' . $_CB_database->NameQuote( 'alias' ) . ' ) = ' . $_CB_database->Quote( strtolower( trim( $username ) ) ) . ' )';
		}
	}
...
function login( $username = null, $password = null, $secretKey = null ) {
	...
	if ( ( ! $username ) || ( ! $password ) || ( ! $secretKey ) ) {
		...
		if ( ! $password ) {
			...
			$password			=	trim( cbGetParam( $_POST, 'password', '', _CB_ALLOWRAW ) );
		}
