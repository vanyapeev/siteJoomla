...
******************************/
	...
	static function userEdit( $user, /** @noinspection PhpUnusedParameterInspection */ $option, $submitvalue, $regErrorMSG = null ) {
		...
		$tabs						=	new cbTabs( true, 1, null, ( $tabbed ? true : false ) );
		...
		$return						.=		( $title ? '<div class="page-header"><h3>' . $title . '</h3></div>' : null )
									.		'<form action="' . $_CB_framework->viewUrl( 'saveuseredit' ) . '" method="post" id="cbcheckedadminForm" name="adminForm" enctype="multipart/form-data" autocomplete="off" class="cb_form form-auto cbValidation">'
...
******************************/
	...
	static function usersList( &$row, &$users, &$columns, &$fields, &$input, $search, $searchmode, $pageNav, &$myUser, &$searchableFields, &$searchValues, &$tabs, $errorMsg, $listAll = true, $random = 0 ) {
		...
		// Base form URL:
		$baseUrl						=	$_CB_framework->viewUrl( 'userslist', true, array( 'listid' => (int) $row->listid, 'searchmode' => 0 ), 'html', 0, '&listid=' . (int) $row->listid );
		...
		$return							=	'<div class="cbUsersList cb_template cb_template_' . selectTemplate( 'dir' ) . ( $pageClass ? ' ' . htmlspecialchars( $pageClass ) : null ) . '">'
										.		'<form action="' . $_CB_framework->viewUrl( 'userslist', true, array( 'listid' => (int) $row->listid, 'searchmode' => (int) $searchmode ), 'html', 0, '&listid=' . (int) $row->listid ) . '" method="post" id="adminForm" name="adminForm" class="cb_form form-auto cbValidation">'
...
static function manageConnections( $connections, $actions, $total, &$connMgmtTabs, &$pagingParams, $perpage, $connecteds = null ) {
	...
	$return							=	'<div class="cbManageConnections cb_template cb_template_' . selectTemplate( 'dir' ) . ( $pageClass ? ' ' . htmlspecialchars( $pageClass ) : null ) . '">'
									.		( is_array( $results ) ? implode( '', $results ) : null )
									.		( $pageTitle ? '<div class="page-header"><h3>' . $pageTitle . '</h3></div>' : null );
	...
	if ( $actions || $connections || ( ( $ueConfig['autoAddConnections'] == 0 ) && $connecteds ) ) {
		...
		if ( $actions ) {
			...
			$return					.=			( $tabCount > 1 ? $tabs->startTab( 'myCon', CBTxt::Th( 'UE_MANAGEACTIONS', 'Manage Actions' ) . ' <span class="badge">' . count( $actions ) . '</span>', 'actions' ) : null )
									.				'<form action="' . $_CB_framework->viewUrl( 'processconnectionactions' ) . '" method="post" id="adminForm" name="adminForm" class="cb_form form-auto cbValidation">'
									.					( $description ? '<div class="form-group cb_form_line clearfix tab_description">' . $description . '</div>' : null )
									.					'<div class="form-group cb_form_line clearfix">';
			...
			foreach( $actions as $action ) {
				...
				$return				.=						'<div class="cbCanvasBox cbCanvasBoxSm img-thumbnail">'
									...
									.									cbTooltip( 1, $tooltip, CBTxt::T( 'UE_CONNECTIONREQUESTDETAIL', 'Connection Request Details' ), 300, null, $cbUser->getField( 'avatar', null, 'html', 'none', 'list', 0, true ), null, 'style="display: block; width: 100%; height: 100%;"' )
									.								'</div>'
									.							'</div>'
									...
									.								'</div>'
									.								'<div class="cbCanvasBoxRow text-nowrap text-overflow">'
									.									moscomprofilerHTML::radioList( $actionList, (int) $action->id . 'action', null, 'value', 'text', 'a', 0, null, false )
		...
		if ( $connections ) {
			$return					.=			( $tabCount > 1 ? $tabs->startTab( 'myCon', CBTxt::Th( 'UE_MANAGECONNECTIONS', 'Manage Connections' ), 'connections' ) : null )
									.				'<form action="' . $_CB_framework->viewUrl( 'saveconnections' ) . '" method="post" id="adminForm" name="adminForm" class="cb_form form-auto cbValidation">'
			...
			foreach( $connections as $connection ) {
				...
				$return				.=							'<tr>'
									...
									.											'</div>'
									.											'<div class="cbCanvasBoxPhoto cbCanvasBoxPhotoLeft text-left">'
									.												cbTooltip( 1, $tooltip, CBTxt::T( 'UE_CONNECTEDDETAIL', 'Connection Details' ), 300, null, $cbUser->getField( 'avatar', null, 'html', 'none', 'list', 0, true ), null, 'style="display: block; width: 100%; height: 100%;"' )
									.											'</div>'
									.										'</div>'
									.										'<div class="cbCanvasBoxBottom bg-default">'
									.											'<div class="cbCanvasBoxRow text-nowrap text-overflow">'
				...
				if ( $connection->accepted && ( ! $connection->pending ) ) {
					$menuItems		=	'<ul class="cbCanvasBoxMenuItems dropdown-menu" style="display: block; position: relative; margin: 0;">'
									.		'<li class="cbCanvasBoxMenuItem"><a href="javascript: void(0);" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'UE_CONFIRMREMOVECONNECTION', 'Are you sure you want to remove this connection?' ) ) . '\' ).done( function() { location.href = \'' . addslashes( $_CB_framework->viewUrl( 'removeconnection', true, array( 'act' => 'connections', 'connectionid' => (int) $connection->memberid ) ) ) . '\'; })"><span class="fa fa-times-circle"></span> ' . CBTxt::T( 'Remove' ) . '</a></li>'
									.	'</ul>';

					$menuAttr		=	cbTooltip( 1, $menuItems, null, 'auto', null, null, null, 'class="cbCanvasBoxMenu btn btn-default btn-xs" data-cbtooltip-menu="true" data-cbtooltip-classes="qtip-nostyle"' );

					$return			.=										'<div class="cbCanvasBoxButtons">'
			...
			foreach( $connecteds as $connected ) {
				...
				if ( ( ! $connected->accepted ) || $connected->pending ) {
					...
					if ( $connected->pending ) {
						$return		.=								'<button type="button" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'UE_CONFIRMREMOVECONNECTION', 'Are you sure you want to remove this connection?' ) ) . '\' ).done( function() { location.href = \'' . addslashes( $_CB_framework->viewUrl( 'removeconnection', true, array( 'act' => 'connections', 'connectionid' => (int) $connected->memberid ) ) ) . '\'; })" class="btn btn-xs btn-danger">' . CBTxt::Th( 'Cancel Request' ) . '</button>';
					} elseif ( ! $connected->accepted  ) {
						$return		.=								'<button type="button" onclick="location.href=\'' . addslashes( $_CB_framework->viewUrl( 'acceptconnection', true, array( 'act' => 'connections', 'connectionid' => (int) $connected->memberid ) ) ) . '\';" class="btn btn-xs btn-success">' . CBTxt::T( 'Accept' ) . '</button>'
									.								' <button type="button" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'UE_CONFIRMREMOVECONNECTION', 'Are you sure you want to remove this connection?' ) ) . '\' ).done( function() { location.href = \'' . addslashes( $_CB_framework->viewUrl( 'denyconnection', true, array( 'act' => 'connections', 'connectionid' => (int) $connected->memberid ) ) ) . '\'; })" class="btn btn-xs btn-danger">' . CBTxt::Th( 'Reject' ) . '</button>';
					}
				...
				if ( $connected->accepted && ( ! $connected->pending ) ) {
					$menuItems		=	'<ul class="cbCanvasBoxMenuItems dropdown-menu" style="display: block; position: relative; margin: 0;">'
									.		'<li class="cbCanvasBoxMenuItem"><a href="javascript: void(0);" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'UE_CONFIRMREMOVECONNECTION', 'Are you sure you want to remove this connection?' ) ) . '\' ).done( function() { location.href = \'' . addslashes( $_CB_framework->viewUrl( 'removeconnection', true, array( 'act' => 'connections', 'connectionid' => (int) $connected->memberid ) ) ) . '\'; })"><span class="fa fa-times-circle"></span> ' . CBTxt::T( 'Remove' ) . '</a></li>'
									.	'</ul>';
