...
switch ( $mode ) {
	case 5:
		$cbVersion					=	$_PLUGINS->getPluginVersion( 1, 2, $feedDuration );
		$messages					=	array();

		if ( $cbVersion[2] === false ) {
			modCBAdminHelper::enableUpdateSite();

			$latestVersion			=	'<span class="cbUpdateVersion label label-danger">' . $cbVersion[1] . '</span>';
			...
			$updateButton			=	'<a href="' . htmlspecialchars( $cbVersion[3] ) . '"><button class="btn btn-primary cbUpdateButton">' . CBTxt::T( 'Update Now' ) . '</button></a>';
